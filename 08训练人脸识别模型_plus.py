...
def GetFaceTrain(filePath):
    ...
    for i in range(len(imagePaths)):
        ...
        # 第二级
        # 获取个人目录下所有的图片
        names_single = os.listdir(path)
        for j in names_single:
            id = int(j.split('.')[0])
            # ids.append(int(result[0]))
            path_new = path + '/' + j
            # (1)打开图像
            # print(path_new)
            # 读取中文目录
            image = cv.imdecode(np.fromfile(path_new, dtype=np.uint8), cv.IMREAD_COLOR)
            # print(image)
            # (2)把图片转换为 灰度图
...
if __name__ == '__main__':
    # 1、获取训练集
    x_train, y_train = GetFaceTrain('./pic')
    # 2、建立人脸识别模型
